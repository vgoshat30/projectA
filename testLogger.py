...
An intuition on the usage of this module will be as follows:
    ...
    2.  Once you have a .mat file with correct specifications to serva as a test
...
class testlogger:
    ...
    def __init__(self, name):
        ...
        fields are created (for example, aCoefs, bCoefs, tanhSlope and
        tanhStretch for a testlogger of type 'tanh')
        ...
        if self.loggerType == 'tanh':
            ...
            # "Slope" of sum of tanh function
            self.tanhSlope = pythonMatFile['tanhSlope']
            # "Stretch" of sum of tanh function
            self.tanhStretch = pythonMatFile['tanhStretch']
            ...
            if self.rate.shape[0]:
                ...
                self.tanhSlope = self.tanhSlope[0]
                self.tanhStretch = self.tanhStretch[0]
    ...
    def addEmptyTest(self):
        ...
        if self.rate.all():
            ...
            if self.loggerType == 'tanh':
                ...
                # "Slope" of sum of tanh function
                self.tanhSlope = np.append(self.tanhSlope,
                                           np.empty((1, 1), float))
                # "Stretch" of sum of tanh function
                self.tanhStretch = np.append(self.tanhStretch,
                                             np.empty((1, 1), float))
    ...
    def log(self, test=None, **kwargs):
        ...
        for key in kwargs:
            ...
            if self.loggerType == 'tanh':
                ...
                if key is 'slope':
                    self.tanhSlope[testIndex] = kwargs[key]
                if key is 'stretch':
                    self.tanhStretch[testIndex] = kwargs[key]
    ...
    def delete(self, test=None, indexing=None):
        ...
        if not(test is None):
            ...
            if self.loggerType == 'tanh':
                ...
                self.tanhSlope = np.delete(self.tanhSlope, deleteIndex, 0)
                self.tanhStretch = np.delete(self.tanhStretch, deleteIndex, 0)
            ...
            if self.loggerType == 'tanh':
                ...
                self.tanhSlope = np.empty((0, 1), float)  # MATLAB Array
                self.tanhStretch = np.empty((0, 1), float)  # MATLAB Array
    ...
    def content(self, test=None):
        ...
        if test is None:
            ...
            for ii, currTest in enumerate(testNum):
                ...
                if self.loggerType == 'tanh':
                    if not(type(self.aCoefs[currTest-1]) is list):
                        aCoefsToPrint = dontExistMessage
                    else:
                        aCoefsToPrint = self.aCoefs[currTest-1]

                    if not(type(self.bCoefs[currTest-1]) is list):
                        bCoefsToPrint = dontExistMessage
                    else:
                        bCoefsToPrint = self.bCoefs[currTest-1]

                    if not self.tanhSlope[currTest-1]:
                        tanhSlopeToPrint = dontExistMessage
                    else:
                        tanhSlopeToPrint = self.tanhSlope[currTest-1]

                    if not self.tanhStretch[currTest-1]:
                        tanhStretchToPrint = dontExistMessage
                    else:
                        tanhStretchToPrint = self.tanhStretch[currTest-1]
                ...
                if self.loggerType == 'normal':
                    ...
                    print("\n\nTest {} Info\n\n"
                          ...
                          .format(currTest, self.rate[currTest-1],
                                  ...
                                  tanhSlopeToPrint, tanhStretchToPrint,
                                  runtimeToPrint, epochToPrint,
                                  removeCellFormat(self.logtime[currTest-1]),
                                  noteToPrint))
    ...
    def save(self):
        ...
        if self.loggerType == 'tanh':
            ...
            saveVars['tanhSlope'] = self.tanhSlope
            saveVars['tanhStretch'] = self.tanhStretch
...
def createMatFile(name, type, theoryRate, theoryLoss):
    ...
    if type is 'tanh':
        ...
        variables['tanhSlope'] = np.empty((0, 1), float)  # MATLAB Array
        variables['tanhStretch'] = np.empty((0, 1), float)  # MATLAB Array
